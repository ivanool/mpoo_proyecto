...
public class CasaInteligente {
    ...
    public static int mostrarMenuPrincipal() {
        String[] MenuPpal = {"1. Ingresar cuarto", "2. Ingresar dispositivos a conectar", "3. Encender dispositivos", "4. Apagar dispositivos", "5. Mostrar dispositivos en habitaciones","6. Eliminar dispositivo", "7. Salir"};
    ...
    public static void eliminarDispositivoDeHabitacion(ArrayList<Habitaciones> listaCuartos, ArrayList<DispositivosInteligentes> listaDispTot) {
        if (listaCuartos.isEmpty() || listaDispTot.isEmpty()) {
            JOptionPane.showMessageDialog(null, "No hay cuartos registrados o dispositivos registrados para realizar esta acción.", "Casa Inteligente", JOptionPane.ERROR_MESSAGE);
            return;
        }

        int idDispositivo = verNumero("Ingrese el ID del dispositivo a eliminar de la habitación: ");
        ...
        for (Habitaciones cuarto : listaCuartos) {
            for (DispositivosInteligentes dispositivo : cuarto.getDispositivosInteligentes()) {
                if (dispositivo != null && dispositivo.getId() == idDispositivo) {
                    ...
                    JOptionPane.showMessageDialog(null, "El dispositivo ha sido eliminado de la habitación " + cuarto.getNombre(), "Casa Inteligente", JOptionPane.INFORMATION_MESSAGE);
                    return;
                }
            }
        }

        JOptionPane.showMessageDialog(null, "No se encontró el dispositivo de ID " + idDispositivo + " en ninguna habitación.", "Casa Inteligente", JOptionPane.ERROR_MESSAGE);
    }
}